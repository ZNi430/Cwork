...
int battleLogic(Monster *m, int tx, int ty) {
    ...
    int rounds = (m->hp + dmgM - 1) / dmgM;//向上取整 
...
void drawMap() {
    ...
    for (int i = 0; i < MAP_SIZE; i++) {
		...
		if (i == 1) printf("   %-16s %s", "[ 勇者状态 ]", "[ 图例说明 ]");
		if (i == 2) printf("   生命: " RED "%-7d" RESET "    " RED "M" RESET ": 史莱姆      " RED "B" RESET ": 蝙蝠", hero.hp);
		if (i == 3) printf("   攻击: " MAGENTA "%-7d" RESET "    " MAGENTA "R" RESET ": 力量药剂    " BLUE "G" RESET ": 覆甲药剂", hero.atk);
		if (i == 4) printf("   防御: " BLUE "%-7d" RESET "    " RED "A" RESET ": 生命药剂    " YELLOW "K" RESET ": 钥匙", hero.def);
		if (i == 5) printf("   金币: " YELLOW "%-7d" RESET "    " YELLOW "D" RESET ": 铁门        " GREEN "$" RESET ": 神秘商店", hero.gold);
		if (i == 6) printf("   钥匙: " YELLOW "%-7d" RESET "    " CYAN "U" RESET ": 上楼梯      " CYAN "N" RESET ": 下楼梯", hero.key);
		if (i == 7) printf("   %-16s " GREEN "V" RESET ": 终点        #: 墙壁", "");
        printf("\n");
    }
    printf("----------------------------------------\n");
...
void handleInput() {
    ...
    if (ch == '\r') {
        if (hero.under_char == 'U' && hero.floor < MAX_FLOOR - 1) {
            ...
            hero.floor++;
            if (hero.floor == MAX_FLOOR-2) strcpy(global_msg, "感受到一股强大的威压，胜利就在前方！");
    ...
    switch (target) {
        ...
        case 'U': sprintf(global_msg, "按回车进入下一层。"); movePlayer(nextX, nextY, 'U'); break;
        case 'N': sprintf(global_msg, "按回车一层。"); movePlayer(nextX, nextY, 'N'); break;
