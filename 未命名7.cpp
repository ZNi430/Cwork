...
void saveGame() {
    ...
    fclose(fp);
    strcpy(global_msg, GREEN "进度已保存到 save.dat！" RESET);
}
...
void drawMap() {
    ...
    printf(CYAN "[W/A/S/D] 移动  [Enter] 交互  [S] 保存存档\n" RESET);
    printf("----------------------------------------\n");
}
...
void handleInput() {
    ...
    if (ch == 'w' || ch == 'W') nextX--; else if (ch == 'x' || ch == 'X') nextX++; // 注意：原代码S被我改成了x用于移动，因为S现在是存档。
    // 为了完全保留你的操作逻辑，这里我将存档逻辑设为检测大写 'S'，或者你可以选择其他键。
    // 下面恢复你的 w/a/s/d 逻辑，但加入存档检测。
    
    // 重新修正：w/a/s/d 移动，按下 'P' (Preserve) 存档，或者坚持用 'S'。
    // 为了不冲突，我们规定：方向键正常，按下 'T' 键存档。
    if (ch == 'w' || ch == 'W') nextX--; 
    else if (ch == 's' || ch == 'S') {
        // 如果是想向下移动，我们需要判断。由于魔塔通常不追求极速，我们改用 [P] 存档。
    }

    // --- 最终决定：为了不破坏你的习惯，w/a/s/d 依然是移动。存档改为按 [P] 键 ---
...
int main() {
    ...
    while (1) {
        system("cls");
        printf(CYAN "========================================\n" RESET);
        printf(CYAN "       M A G I C   T O W E R            \n" RESET);
        printf(CYAN "========================================\n\n" RESET);
        printf(YELLOW "          1. 开始新游戏\n\n" RESET);
        printf(GREEN  "          2. 读取旧存档\n\n" RESET);
        printf(RED    "          3. 退出游戏\n\n" RESET);
        printf("       请选择 [1-3]: ");
        ...
        if (choice == '2') {
            if (loadGame()) {
                strcpy(global_msg, "存档读取成功！欢迎回来。");
                break;
            } else {
                printf(RED "\n\n  未找到存档文件 (save.dat)！" RESET);
                _getch();
            }
        }
